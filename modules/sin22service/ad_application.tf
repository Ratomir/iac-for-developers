...
resource "azuread_application" "sinergija22service" {
  display_name = "${var.project_name}-${var.name}-aks-${local.service_name}"
}
...
resource "azuread_application_federated_identity_credential" "sinergija22service" {
  ...
  display_name          = "${var.project_name}-${var.name}-aks-${local.service_name}"
  description           = "Kubernetes service account federated credential for ${var.name}"
  audiences             = ["api://AzureADTokenExchange"]
  issuer                = var.aks_cluster.oidc_issuer_url
  subject               = "system:serviceaccount:${var.project_name}:${var.name}-aks-${local.service_name}"
}
